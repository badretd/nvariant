...
| `private` | нет | Скрыть материал из всех публичных списков и индексов, сохранив доступ по slug |
...
| `private` | нет | Скрыть выпуск из всех публичных списков и индексов, сохранив доступ по slug |
| `cover` | да | Media ID обложки |
| `sections` | да | Упорядоченные главы |
| `corrections` | нет | История правок выпуска |
| `seo` | нет | SEO override |

### Приватные материалы и выпуски

Чтобы опубликовать материал или выпуск только по прямой ссылке, добавьте в его запись:
...
- открывается по обычному адресу `/materials/<slug>` или `/issues/<slug>`;
- не показывается на главной, в архивах, поиске, RSS, sitemap, списках связанных материалов и местах использования медиа;
- получает мета-директиву `noindex, nofollow`.

Это режим «доступ по ссылке», а не защита доступа: любой пользователь, знающий slug, сможет открыть страницу и передать ссылку другому. Для секретных материалов нужна отдельная авторизация.

Не добавляйте приватный материал в публичный выпуск: содержимое выпуска доступно целиком на его странице. Если весь выпуск должен быть доступен только по ссылке, поставьте `private: true` и выпуску, и каждому его материалу. Поле `showInMaterialsArchive` управляет только архивом `/materials` и не заменяет `private`.
...
Публичный материал попадёт в RSS после добавления в `materials`. Записи с `private: true` исключаются.

Публичный выпуск попадёт в RSS после добавления в `issues`. Записи с `private: true` исключаются.
...
- все публичные issues;
- все публичные materials;
- все media.

Приватные материалы и выпуски в sitemap не добавляются.
